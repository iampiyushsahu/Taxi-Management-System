...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['book1'])) {
        ...
        if($number_of_members > $z['taxi_seats']){
            ...
            $a = " INSERT INTO taxi_book_ride1 ( name, your_location, destination, pick_up_time,number_of_members,pin_code) VALUES ( '$name', ' $your_location', '$destination', '$pick_up_time', ' $number_of_members', '$pin_code')";        
            $result = mysqli_query($conn, $a);
            if ($result) {
                if (!empty($name) && !empty($pin_code) && !empty($your_location) && !empty($destination) && !empty($pick_up_time) && !empty($number_of_members)) {
                    $_SESSION['c'] = $name;
                    $a1 = "SELECT * from all_taxis where id = '$id'";
                    $result_all =  mysqli_query($conn, $a1);
                    $fetch = mysqli_fetch_assoc($result_all);
                    $_SESSION['car_name'] = $fetch['taxi_name'];
...
<body>
    ...
    <div class="container">
        ...
        <div class="b11">
            <a href="taxi.php"><button class="b1">Back</button></a>
        </div>
